...
SpreadSheet.prototype.initialize = function(){
    ...
    return new Promise(function(resolve, reject) {
        fs.readFile(spreadsheet_this.spreadsheet_path, function (err, data) {
            ...
            spreadsheet_this.parser.load_excel(new JSZip(data));
            Promise.all([
                spreadsheet_this.parser.parse_file_in_excel('xl/sharedStrings.xml')
                ,spreadsheet_this.parser.parse_file_in_excel('xl/_rels/workbook.xml.rels')
                ,spreadsheet_this.parser.parse_file_in_excel('xl/workbook.xml')
                ,spreadsheet_this.parser.parse_dir_in_excel('xl/worksheets')
...
SpreadSheet.prototype.generate = function(){
    ...
    .file('xl/sharedStrings.xml', builder.buildObject(this.shared_strings.obj));
    var spreadsheet_this = this;
...
SpreadSheet.prototype.copy_sheet = function(
    ...
    this.workbookxml_rels = this._add_sheet_to_workbookxml_rels(next_id);
    this.workbookxml = this._add_sheet_to_workbookxml(next_id, dest_sheet_name);
    var dest_sheet = {};
    _.extend(dest_sheet, this._get_sheet_by_name(src_sheet_name).value);
    dest_sheet.name = 'sheet'+next_id+'.xml';
    this.sheet_xmls.push(dest_sheet);
    return;
};
...
SpreadSheet.prototype.bulk_copy_sheet = function(
    ...
    var spreadsheet_this = this;
    _.each(dest_sheet_names, function(sheet_name){
        spreadsheet_this.copy_sheet(src_sheet_name, sheet_name);
    });
    return;
};
...
SpreadSheet.prototype.add_row = function(
    ...
    return this.bulk_add_value(sheetname, key_values,existing_setting);
}
...
SpreadSheet.prototype.add_value = function(
    ...
    var row_string = util.get_row_string(cell_name);
    var row = spreadsheet_this._get_row_by_name(
        sheetname,
        row_string
    );
...
SpreadSheet.prototype._update_row = function(
    ...
    row.c = _.sortBy(row.c, function(e){return util.revert_number(util.get_col_string(e['$'].r))});
