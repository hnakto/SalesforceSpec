...
SpreadSheet.prototype.initialize = function(){
    ...
    return new Promise(function(resolve, reject) {
        fs.readFile(spreadsheet_this.spreadsheet_path, function (err, data) {
            ...
            parser.parseString(
                spreadsheet_this.zip.file('xl/sharedStrings.xml').asText(),
                function (err, shared_strings) {
                    if (err) {
                        reject(err);
                    }
                    spreadsheet_this.shared_strings = new SharedStrings(shared_strings);
                    resolve();
                }
            )
...
SpreadSheet.prototype.bulk_add_value = function(
    ...
    return new Promise(function(resolve, reject){
        key_values.reduce(
            ...
            function(result) {
                resolve(spreadsheet_this.generate());
            }
        ).catch(
            function(err){
                reject(err);
