...
SpreadSheet.prototype.copy_sheet = function(
    ...
    return new Promise(function(resolve, reject) {
        ...
        .file("xl/_rels/workbook.xml.rels",builder.buildObject(spreadsheet_this.workbookxml_rels))
        .file("xl/workbook.xml",builder.buildObject(spreadsheet_this.workbookxml))
        .file('xl/worksheets/sheet'+next_id+'.xml',spreadsheet_this._get_sheet_by_name(src_sheet_name).value);
...
SpreadSheet.prototype.add_row = function(
    ...
    log.debug('SpreadSheet:add row ' + row_number + ' on ' + sheetname);
    var key_values = [];
...
SpreadSheet.prototype.bulk_add_value = function(
    ...
    return Promise.resolve()
        .then(function(sheet_xml_obj){
            var sheet_xml_obj = spreadsheet_this._get_sheet_by_name(sheetname);
            return new Promise(function(resolve, reject) {
                key_values.reduce(
                    function (promise, cell) {
                        return promise.then(function (value) {
                            return spreadsheet_this.add_value(
                                sheet_xml_obj,
                                sheetname,
                                cell.cell_name,
                                cell.value
            ...
            log.error('Error while adding value bulkly');
            log.error(err);
        });
