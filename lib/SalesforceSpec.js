                ...
                tmp.dir({dir: config.temporary_directory},function(err, dir) {
                    ...
                    var pipe_result = spec.conn.metadata.retrieve({unpackaged: package})
                        ...
                        fs.createWriteStream(
                            ...
                            spec.ext_dir = './' + dir +'/'+ 'unpackaged/';
                            log.info('SalesforceSpec:metadata is successfully retrieve.');
                            resolve();
                        }).on('error',function(e){
                            reject(e);
                        });
...
SalesforceSpec.prototype.parse_directory = function(metadata_type){
    ...
    return fs.readdirAsync(spec.ext_dir + '/' + metadata_type + '/')
    .map(function(file_name_with_extension){
        var file_name = file_name_with_extension.match(/(.+)(\.[^.]+$)/)[1];
        return fs.readFileAsync(spec.ext_dir + '/' + metadata_type + '/' + file_name_with_extension, 'utf8')
...
SalesforceSpec.prototype.custom_object_summary = function(){
    ...
    _.each(this.objects, function(obj){
        if(obj.file_content.CustomObject.label 
            && obj.file_content.CustomObject.description 
            && obj.file_content.CustomObject.fullName){
            all_object.push({
                label:obj.file_content.CustomObject.label[0],
                fullName:obj.file_content.CustomObject.fullName[0],
                description:obj.file_content.CustomObject.description[0]
            });
        }
    });
    return all_object;
