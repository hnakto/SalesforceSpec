...
SalesforceSpec.prototype.initialize = function(){
    ...
    var temporary_directory = process.env.TEMPORARY_DIRECTORY;
    ...
    return this.conn.login(username,password)
        .then(function(){
            ...
            return spec_this.parse_all_metadata();
        });
}
...
SalesforceSpec.prototype.retrieve_package = function(){
    ...
    return Promise.props({
            objs:spec_this.list_metadata('CustomObject'),
            layouts:spec_this.list_metadata('Layout'),
            profiles:spec_this.list_metadata('Profile')
        }).then(function(meta){
            ...
            return new Promise(function(resolve, reject){
                tmp.dir({dir:process.env.TEMPORARY_DIRECTORY},function(err, dir) {
                    if (err)
                        reject(err);
...
SalesforceSpec.prototype.parse_all_metadata = function(){
    var spec_this = this;
    return Promise.props({
        layouts: spec_this.parse_directory('layouts'),
        objects: spec_this.parse_directory('objects'),
        profiles: spec_this.parse_directory('profiles'),
        workflows: spec_this.parse_directory('workflows')
    }).then(function(meta){
        meta.objects = _.filter(meta.objects, function(object){
            return (object.file_name.indexOf('__c') !== -1) 
                && (object.file_content.CustomObject.customSettingsType === undefined);
        });
        meta.profiles = _.filter(meta.profiles, function(profile){
            ...
            return checked1 || checked2 ? true : false;
        });
        spec_this.layouts = meta.layouts;
        spec_this.objects = meta.objects;
        spec_this.object_names = _.map(meta.objects,function(e){return e.file_name;});
        spec_this.profiles = meta.profiles;
        spec_this.workflows = meta.workflows;
        return;
    });
}
...
SalesforceSpec.prototype.object_label = function(){
    ...
    _.each(this.objects, function(obj){
        object_label[obj.file_name] = obj.file_content.CustomObject.label[0];
    });
    return object_label;
};
...
SalesforceSpec.prototype.custom_field = function(){
    ...
    _.each(this.objects, function(obj) {
        ...
        _.each(obj.file_content.CustomObject.fields, function(field){
            ...
            field_entry.type = type_label[field.type[0]];
...
SalesforceSpec.prototype.valid_profile = function(){
    ...
    _.each(Object.keys(this.object_permission()), function(profilename){
         var cruds = spec_this.object_permission()[profilename];
...
SalesforceSpec.prototype.list_metadata = function(metadata_type){
    ...
    return Promise.resolve()
        .then(function(){
            return spec_this.conn.metadata.list([{type: metadata_type}]);
        }).map(function(e){
            return e.fullName;
        });
